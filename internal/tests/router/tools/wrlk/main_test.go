...
func TestWrlkGuideCommand_PrintsOperationalGuide(t *testing.T) {
	...
	assert.Contains(t, result.stdout, "ext install")
	assert.Contains(t, result.stdout, "ext remove")
	assert.Contains(t, result.stdout, "ext app add")
	assert.Contains(t, result.stdout, "ext app remove")
...
func TestWrlkGuideCurrentCommand_PrintsCurrentInventory(t *testing.T) {
	...
	assert.Contains(t, result.stdout, "PortCLIChrome = \"cli-chrome\"")
	assert.Contains(t, result.stdout, "PortCLIInteraction = \"cli-interaction\"")
	assert.Contains(t, result.stdout, "PortCLIStyle = \"cli-style\"")
	assert.Contains(t, result.stdout, "Optional capability extensions:")
	assert.Contains(t, result.stdout, "charmcli (optional)")
	assert.Contains(t, result.stdout, "prettystyle (optional)")
	assert.Contains(t, result.stdout, "provides: PortCLIChrome, PortCLIInteraction")
	assert.Contains(t, result.stdout, "provides: PortCLIStyle")
	assert.Contains(t, result.stdout, "consumes: none")
	assert.Contains(t, result.stdout, "summary: Package charmcli is a router capability extension")
	assert.Contains(t, result.stdout, "summary: Package prettystyle is a router capability extension")
	assert.Contains(t, result.stdout, "usage: Depend on this extension when callers want styled CLI text, tables, or simple semantic layouts.")
	assert.Contains(t, result.stdout, "usage: Depend on this extension when callers need semantic CLI text or layout")
	assert.Contains(t, result.stdout, "usage: Depend on this extension when callers need semantic CLI prompts")
